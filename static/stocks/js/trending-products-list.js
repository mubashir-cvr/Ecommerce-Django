...
function LoadTrendingProducts() {
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#pageHeading').html('<a href="#" onclick=LoadTrendingProducts(' + $("#subsubcategoryID").val() + ')>' + $("#subsubcategoryName").val() + '</a>')
            return 0;
...
function deleterecord(id) {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    url: "/stockapi/trendingproducts/" + id,
                    type: 'DELETE',
                    dataType: "JSON",
